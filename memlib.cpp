...
int_t mem_manip_lib::mem_read_int(bool signedness) const {
    if (signedness) {
        switch (sizeof(int_t)) {
            case 1:
                return mem_read_byte(1);
            case 2:
                return *static_cast<std::int16_t*>(this->mem_address);
            case 4:
                return *static_cast<std::int32_t*>(this->mem_address);
            case 8:
                return *static_cast<std::int64_t*>(this->mem_address); 
        }
    }
    switch (sizeof(int_t)) 
    {
        case 1:
            return mem_read_byte(0);
        case 2:
            return *static_cast<std::uint16_t*>(this->mem_address);
        case 4:
            return *static_cast<std::uint32_t*>(this->mem_address);
        case 8:
            return *static_cast<std::uint64_t*>(this->mem_address);
    }
}
